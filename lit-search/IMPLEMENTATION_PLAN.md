...
## 阶段 11｜接入判定 API（TypeSafe Jev）

本以为是"换个更便宜的筛选后端"，实测下来不是。完整推导见
`learnings/2026-09-20-一个判定器不是一个后端.md`。

新增：`lit screen --model jev`、`lit screen-eval`、`lit triage`、`lit facets`。
854 项测试全绿，ruff 干净。

### 50｜同一套阈值在两个课题上结果相反

| 课题 | AUC | 人工队列 | 现有 LLM 基线 |
|---|---|---|---|
| pediatric（197 条，对照真人裁决） | 0.987 | **11.2%** | 18.3% |
| ISLES（21,712 条，分层抽样按真实患病率投影） | 0.925 | 8.5% | **7.2%** |

同一套代码、同一套阈值、同一个模型版本。ISLES 上失效的原因查得到：
`X1`「仅做出血性卒中……**且无**缺血性卒中数据」这类多跳间接 + 隐含否定的标准，
单看它的 AUC 只有 0.675，基本没有判别力——正是官方 jaggedness 文档点名的失败模式。

结论：**判定质量是任务与判定器的交互，不是判定器的属性。** 不设通用默认阈值，
配置文件必填，放量前必须过三条准入（金标 100% 保留、队列 ≤15%、不劣于本课题基线）。

### 51｜项目原本没有任何筛选精度指标

`metrics.py` 量的是**检索**召回（该找到的找到没有），没有一个指标回答**筛选**
准不准。于是"换模型是不是更好"只能凭感觉。评估基座（`screen_eval.py` +
`lit screen-eval`）因此从方案末位提到了首位，成了后面每一个阈值决定的依据。

三条纪律都是被数据逼出来的：金标银标分开报（实测某课题银标纳入 21% 被真人推翻）；
每个比率带 Wilson 区间（44 条纳入即使 100% 敏感，下界也只有 92%）；
不可逆漏排与进队列分开数（前者那篇论文再也不会被看到，后者只是多读一篇全文）。

### 52｜退步守卫用错了判据，而且只在续跑时生效

`learnings/2026-07-30` 定的规则是无条件的，实现却有两层缺口。判据「判定条数比
上一轮少」测不到它要防的失败：`merge_verdicts(expected_keys=by_key)` 保证每条
记录都有判定，一轮全灭表现为「每条都有判定，但全是待人工」，**条数分毫不变**。
当年那次事故能表现成「0 条判定」，是因为当时还没有 `expected_keys`。
改判据为**已定谳集合的丢失**，并覆盖普通重跑（此前 `previous` 只在 `--resume`
时加载）。连带修：人工裁决此前也只在续跑时盖回，普通重跑会让模型判定悄悄覆盖
人工结论。

### 53｜把判定器映射成第三个通道会坏事

一条明显离题的记录 p_inc=0.03、p_exc=0.02：纳入视角判 exclude，排除视角
「找不到排除理由」判 include，`merge_verdicts` 判成分歧送进人工队列。要避免就得
让第二个通道也读 p_inc，那两个通道就成了同一向量的函数——正是 `screen.py` 明令
禁止的「同一个偏差重复两次」。所以判定器是**打分器不是通道对**：每条记录一个
`Verdict`，由纯函数 `route` 直接构造 `MergedDecision`。

### 54｜排除标准参与打分反而更差，但它确实带信号

乘上 `(1 - max 排除概率)` 让 AUC 从 0.989 掉到 0.981——官方文档也说一个问题与其
否定式的概率不满足互补关系。所以默认只标注不打分。但真实数据里有一条社论：
判定器给出 `X4=0.92`（**正确识别为社论**），这个判断被 annotate 模式丢掉了，
最后按纳入标准判了纳入。查了全部 23 条高 `X4` 记录：硬否决能省 5 条队列，
要付最多 2 条不可逆误排——按项目取舍不划算，维持现状，但记下这个反例。

### 55｜respx 测不出事件循环绑定

十项 CLI 测试全绿，真跑第一次抛 `Event loop is closed`：httpx 的连接池绑在创建它
的事件循环上，而我分了两次 `asyncio.run`。respx 在传输层拦截、根本不建真实连接，
所以一个都没测出来。补回归测试时先把旧写法忠实还原、确认它真的以同一个报错失败，
再改回来——否则补的可能是一个永远不会红的测试。

### 56｜token 随问题数亚线性增长

1 问 630、3 问 674、8 问 784、16 问 966。state 占大头，每多一个问题只加约 20
token。官方「一次请求问很多问题」的建议在成本上成立：八条标准全问一遍，相对
只问一条只贵 24%。

### 57｜分诊比替换更有价值

两个历史 run 合计 **2,541 行人工队列至今空着**。问题不在人不愿看，在于一千多行
没有顺序。ISLES 1,568 条实测：1 分 42 秒、约 $0.05，其中 98 条 ≥0.7、981 条 <0.1。
这是跨课题都成立的用法——即使在 Jev 不该做全量筛选的 ISLES 上，它排队列依然有效。
