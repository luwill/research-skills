...
## 第三种后端：判定 API（`--model jev`）

TypeSafe Jev 不生成文本。给它一个 `state` 和一组 typed question，它并行、互相
隔离地逐条作答，返回**校准过的概率**。现有双通道 LLM 里那个决定是否送人工的
`confidence`，是模型自报的浮点数；这里换成了真概率。

**但它不是一个可以设默认值的通用后端。** 实测同一套代码、同一套阈值、同一个
模型版本，在两个课题上的结果相反：

| 课题 | AUC | 人工队列 | 现有 LLM 基线 | 结论 |
| --- | --- | --- | --- | --- |
| pediatric（197 条，对照真人裁决） | 0.987 | **11.2%** | 18.3% | 通过准入 |
| ISLES（21,712 条，分层抽样投影） | 0.925 | 8.5% | **7.2%** | 不通过 |

ISLES 上失效的原因查得到：`X1`「仅做出血性卒中……**且无**缺血性卒中数据」这种
多跳间接 + 隐含否定的标准，单看它的 AUC 只有 0.675，基本没有判别力——这正是
官方 jaggedness 文档点名的失败模式。

所以**阈值逐课题调，没有通用默认值**，配置文件必填：

```bash
# 1. 先评估：拿已有的人工裁决轮次当参考，量化这个课题上能不能用
uv run lit screen-eval <run> --topic ... --reference-round 3     --candidate-scores jev_scores.jsonl --score-keys I1,I2

# 2. 过了准入再放量（三条门槛：金标 100% 保留、队列 ≤15%、不劣于本课题基线）
uv run lit screen <run> --topic ... --model jev --jev-profile topics/<id>.jev.yaml
```

配置示例见 `topics/pediatric-intestinal-obstruction-ai.jev.yaml`，里面记着这个
课题的实测数字。价目 $0.042/百万输入 token、输出免费：ISLES 全量约 $1.10，
对比实测 deepseek-v4-pro 的 $9.13。

### 打分与判定分离

网络调用只发生一次，结果落成 `jev_scores.jsonl`；改阈值、扫阈值、分诊、评估
全部是对同一批打分**重新路由**，零成本、零网络不确定性。

### 三个新命令

| 命令 | 用途 |
| --- | --- |
| `lit screen-eval` | 量化一个判定器：金标/银标分开报、Wilson 区间、校准表、阈值扫描、分歧清单 |
| `lit triage` | 给人工队列排优先级：按最可能纳入排序，标出争议标准；`--auto-resolve` 可解开僵局 |
| `lit facets` | 给已纳入记录打标注：外部验证、多中心、公开数据集、代码公开、前瞻性 |

`lit triage` 是跨课题都成立的那个用法。ISLES 1,568 条积压队列实测：1 分 42 秒、
约 $0.05，其中 98 条 ≥0.7（很可能该纳入）、981 条 <0.1（很可能该排除）。
队列从"随机顺序的一千多行"变成了有优先级、带线索的工作队列。

`lit facets` 的答案只有 `stated` / `not_stated`，**永远没有 no**——摘要没提
外部验证，不等于这项研究没做。产物是独立文件，不进证据表。
